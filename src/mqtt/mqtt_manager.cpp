...
void MQTTManager::publishState(const SystemState& systemState, const SensorManager& sensors, const RelayManager& relays) const {
  ...
  doc["sensors"]["temperature"] = sensors.getTemperature().value;
  doc["sensors"]["ph"] = sensors.getPH().value;
  doc["sensors"]["do"] = sensors.getDO().value;
  doc["sensors"]["water_level"] = sensors.getWaterLevel().value;
